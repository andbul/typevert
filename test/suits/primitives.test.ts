...
export function suit() {
    ...
    it('should ignore null fields', () => {
        ...
        // Setup
        class A { num_a: Number = null;}
        class B {num_b: Number;}
        const a = new A()
        const converter = generateConverter(A, B, [
    ...
    it('should map fields to one to one', () => {
        ...
        // Assert
        expect(result).to.have.property("num_b").that.equals(1)
        expect(result).to.have.property("str_b").that.equals("foo")
        expect(result).to.have.property("arr_b").that.deep.equals([1, 2, 3])
        expect(result).to.have.property("obj_b").that.deep.equals({ "foo": "foo_value" })
